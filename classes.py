    ...
    def set_ship_user(self, ship, board):
        try:
            ...
            if not (0 < x < board.get_width + 1) or not (0 < y < board.get_height + 1):
                raise ValueError
            if ship.get_len != 1:
                direct = input("Куда смотрит ваш корабль? (U=UP, R=RIGHT, L=LEFT, D=DOWN, RES - начать заново) ")
            else:
                direct = input("Введите RES если хотите начать заново, U - продолжить ")
            if not (direct == 'U' or direct == 'R' or direct == 'L' or direct == 'D' or direct == 'RES'):
                raise ValueError
        except ValueError:
            print("Невозможные кооридинаты, попробуйте снова!")
